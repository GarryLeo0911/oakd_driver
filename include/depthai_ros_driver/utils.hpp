...
struct ImgPublisherConfig {
    ...
    std::string qosReliability = "reliable";     // "reliable" or "best_effort"
    std::string qosDurability = "volatile";      // "volatile" or "transient_local"
    std::string qosHistory = "keep_last";        // "keep_last" or "keep_all"
    int qosDepth = 10;                           // Queue depth
};
}  // namespace utils
}  // namespace depthai_ros_driver
