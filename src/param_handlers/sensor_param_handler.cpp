...
void SensorParamHandler::declareCommonParams(dai::CameraBoardSocket socket) {
    ...
    // QoS parameters for wireless communication
    declareAndLogParam<std::string>("i_qos_reliability", "reliable");
    declareAndLogParam<std::string>("i_qos_durability", "volatile");
    declareAndLogParam<int>("i_qos_history_depth", 10);
}
